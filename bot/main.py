...
bot = BotClient( # Custom bot instance, defined on bot.py
    command_prefix=['aproveita e ', 'Aproveita e ', 'APROVEITA E ', '!!', '<@&1426651445800865953> ', '<@1247668018708742164> '], # prefixes
    intents=intents, # the intents
    send_errors_to_developer_dm= not has_terminal, # the send to dev dm thing
...
def run():
    bot.run(os.environ['TETO_DISCORD_TOKEN']) # runs the bot with the secret discord token used to connect the script to the discord servers
